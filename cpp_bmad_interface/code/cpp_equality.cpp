...
bool operator== (const CPP_wall3d_section& x, const CPP_wall3d_section& y) {
  bool is_eq = true;
    ...
    if (!is_eq) return false;
    if (x.surface) is_eq = (*x.surface == *y.surface);
    is_eq = is_eq && (x.type == y.type);
...
bool operator== (const CPP_ramper_lord& x, const CPP_ramper_lord& y) {
  bool is_eq = true;
    ...
    if (!is_eq) return false;
    if (x.attrib_ptr) is_eq = (*x.attrib_ptr == *y.attrib_ptr);
  return is_eq;
};
...
bool operator== (const CPP_ele& x, const CPP_ele& y) {
  bool is_eq = true;
    ...
    if (!is_eq) return false;
    if (x.descrip) is_eq = (*x.descrip == *y.descrip);
    is_eq = is_eq && (x.a == y.a);
    is_eq = is_eq && (x.b == y.b);
    is_eq = is_eq && (x.z == y.z);
    is_eq = is_eq && (x.x == y.x);
    ...
    if (!is_eq) return false;
    if (x.ac_kick) is_eq = (*x.ac_kick == *y.ac_kick);
    is_eq = is_eq && (x.bookkeeping_state == y.bookkeeping_state);
    is_eq = is_eq && (x.control.has_value() == y.control.has_value());
    if (!is_eq) return false;
    if (x.control) is_eq = (*x.control == *y.control);
    is_eq = is_eq && (x.floor == y.floor);
    is_eq = is_eq && (x.high_energy_space_charge.has_value() == y.high_energy_space_charge.has_value());
    if (!is_eq) return false;
    if (x.high_energy_space_charge) is_eq = (*x.high_energy_space_charge == *y.high_energy_space_charge);
    is_eq = is_eq && (x.mode3.has_value() == y.mode3.has_value());
    if (!is_eq) return false;
    if (x.mode3) is_eq = (*x.mode3 == *y.mode3);
    is_eq = is_eq && (x.photon.has_value() == y.photon.has_value());
    if (!is_eq) return false;
    if (x.photon) is_eq = (*x.photon == *y.photon);
    is_eq = is_eq && (x.rad_map.has_value() == y.rad_map.has_value());
    if (!is_eq) return false;
    if (x.rad_map) is_eq = (*x.rad_map == *y.rad_map);
...
bool operator== (const CPP_lat& x, const CPP_lat& y) {
  bool is_eq = true;
    ...
    if (!is_eq) return false;
    if (x.n_ele_track) is_eq = (*x.n_ele_track == *y.n_ele_track);
    is_eq = is_eq && (x.n_ele_max.has_value() == y.n_ele_max.has_value());
    if (!is_eq) return false;
    if (x.n_ele_max) is_eq = (*x.n_ele_max == *y.n_ele_max);
